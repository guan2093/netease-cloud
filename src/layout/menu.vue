<template>
   <div class="menu">
    ...
    <div :key="index" class="menu-block" v-for="(menu, index) in menusWithPlaylist">
      ...
      <ul class="menu-list">
        <router-link 
        :key="index" :to="item.path" active-class="menu-item.active" 
        class="menu-item" tag="li" v-for="(item, index) in menu.children">
...
export default {
   ...
   computed: {
    // 组合登录后的歌单
    menusWithPlaylist(){
      return this.isLogin & userMenus.length ? this.menus.concat(this.userMenus) : this.menus
    },
